...
func TestGeneratePatternsDoc(t *testing.T) {
	...
	f.WriteString("# Patterns documentation\n\n")
	f.WriteString("You will find here a generated documentation of all the patterns loaded by crowdsec.\n")
	f.WriteString("They are sorted by pattern length, and are meant to be used in parsers, in the form %{PATTERN_NAME}.\n")
	f.WriteString("\n\n")
	for _, k := range p {
	...
	f.WriteString("\n")
	f.WriteString("# Documentation generation\n")
	f.WriteString("This documentation is generated by `pkg/parser` : `GO_WANT_TEST_DOC=1 go test -run TestGeneratePatternsDoc`\n")
	f.Close()
