...
func InstallHubItems(ctx context.Context, hub *cwhub.Hub, hubProvider cwhub.HubProvider, input []byte, yes, dryRun, verbose bool) error {
	...
	for _, setupItem := range setupEnvelope.Setup {
		...
		for _, collection := range setupItem.Install.Collections {
			...
			plan.AddCommand(hubops.NewDownloadCommand(item, hubProvider, forceAction))
			if !downloadOnly {
