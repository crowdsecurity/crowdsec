...
func (l *VLSource) readOneEntry(entry *vlclient.Log, labels map[string]string, out chan types.Event) {
	...
	if l.metricsLevel != metrics.AcquisitionMetricsLevelNone {
		victorialogs_metrics.VictorialogsDataSourceLinesRead.With(prometheus.Labels{"source": l.Config.URL, "datasource_type": "victorialogs", "label_type": l.Config.Labels["type"]}).Inc()
	}
