...
// The TokenItemFunc type is an adapter to allow the use of ordinary
// function as TokenItem mutator.
type TokenItemFunc func(context.Context, *ent.TokenItemMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f TokenItemFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.TokenItemMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.TokenItemMutation", m)
}
