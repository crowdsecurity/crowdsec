...
func TestNewChallengeRuntimeStartupBudget(t *testing.T) {
	...
	// The cache must hold at least the seeded bundle so the very first request
	// can be served.
	got := rt.getCachedChallengeJS()
	require.NotEmpty(t, got.Code, "cache should be seeded with the baked-in initial bundle")
}
...
func TestSeedCacheFromInitialBundle(t *testing.T) {
	...
	rt.cacheMutex.RLock()
	defer rt.cacheMutex.RUnlock()
	require.Len(t, rt.obfuscatedJSCache, 1)
	require.NotEmpty(t, rt.obfuscatedJSCache[0].Code)
	...
	t.Logf("seeded variant size: %d bytes", len(rt.obfuscatedJSCache[0].Code))
	require.Greater(t, len(rt.obfuscatedJSCache[0].Code), 100_000,
	...
	require.NotContains(t, rt.obfuscatedJSCache[0].Code, "__CROWDSEC_SUBMIT_PATH__",
		"submit path placeholder still present; substitution missed at build time")
	require.NotContains(t, rt.obfuscatedJSCache[0].Code, "__CROWDSEC_POW_WORKER_PATH__",
