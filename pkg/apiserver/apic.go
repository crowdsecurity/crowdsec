...
// isAppSecAlert checks if an alert was generated by AppSec WAF.
// All AppSec alerts have messages starting with "WAF block:" or "WAF out-of-band match:"
// as set in pkg/acquisition/modules/appsec/utils.go:AppsecEventGeneration
func isAppSecAlert(alert *models.Alert) bool {
	if alert == nil || alert.Message == nil {
		return false
	}
	return strings.HasPrefix(*alert.Message, "WAF block:") ||
		strings.HasPrefix(*alert.Message, "WAF out-of-band match:")
}
...
func shouldShareAlert(alert *models.Alert, consoleConfig *csconfig.ConsoleConfig, shareSignals bool) bool {
	...
	// Check if this is an AppSec alert and handle specially
	if isAppSecAlert(alert) {
		if consoleConfig.ShareAppSecAlerts != nil && *consoleConfig.ShareAppSecAlerts {
			log.Debugf("AppSec alert (id:%d) should be shared (share_appsec enabled)", alert.ID)
			return true
		}
		log.Debugf("AppSec alert (id:%d) should not be shared (share_appsec disabled or not configured)", alert.ID)
		return false
	}
