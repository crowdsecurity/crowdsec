...
func registerPrometheus(config *csconfig.PrometheusCfg) {
	...
	if config.ListenAddr == "" {
		log.Errorf("prometheus is enabled, but the listen address is empty, using '127.0.0.1'")
		config.ListenAddr = "127.0.0.1"
	}
	if config.ListenPort == 0 {
		log.Errorf("prometheus is enabled, but the listen port is empty, using '9090'")
