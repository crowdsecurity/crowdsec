...
* `$AGENT_USERNAME`         - Agent username (to register if is LAPI or to use if it's an agent)
* `$AGENT_PASSWORD`         - Agent password (to register if is LAPI or to use if it's an agent)
