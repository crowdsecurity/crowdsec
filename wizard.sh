...
install_crowdsec() {
    ...
    PID=${PID_DIR} DATA=${CROWDSEC_DATA_DIR} CFG=${CROWDSEC_CONFIG_PATH} envsubst '$CFG $PID $BIN' < ./config/prod.yaml > ${CROWDSEC_CONFIG_PATH}"/default.yaml"   
    PID=${PID_DIR} DATA=${CROWDSEC_DATA_DIR} CFG=${CROWDSEC_CONFIG_PATH} envsubst '$CFG $PID $BIN' < ./config/user.yaml > ${CROWDSEC_CONFIG_PATH}"/user.yaml"
